# 🦖 HƯỚNG DẪN ISLE LIVE MAP

**1. Cài đặt:** Tải bộ cài do nhà phát hành cung cấp, mở file `.exe` và cài đặt. App không tự tải bản cập nhật từ GitHub.

**2. Kết nối:** Chọn đúng nguồn trên Home:

- DinoVietNam / Premium / HoHo / server trên mạng IslePilot → **ĐĂNG NHẬP STEAM** một lần, app tự nhận server đang chơi.
- SDVN #3 (server chạy IslePilot riêng trên `3.sdvn.org`) → nút **SDVN #3 X3 GROW**, đăng nhập Steam một lần cho host này.
- EraGaming → **ERAGAMING**. PANDORA → **PANDORA**.

Hoàn tất đăng nhập ngay trong cửa sổ tool rồi bấm **KIỂM TRA PHIÊN** nếu cửa sổ chưa tự đóng.

**3. Vào game:** Join server rồi giữ tool chạy. Map, hướng quay và status dino sẽ tự cập nhật khi website nguồn đã nhận dữ liệu.

**4. Chơi nhóm (tùy chọn):** Ở Home, nhập tên → **TẠO NHÓM** → gửi mã 6 ký tự. Bạn bè nhập tên + mã → **NHẬP MÃ**. Khi cùng server, overlay sẽ hiện vị trí, hướng quay, HP, Đói và Nước của nhau.

> Nhóm chỉ tồn tại trong phiên. Đóng app thì lần sau phải tạo/nhập mã lại; tool không lưu lịch sử nhóm.

> Texture Gateway đã nằm sẵn trong app. Mạng yếu chỉ làm telemetry chuyển trạng thái reconnect, không phải tải lại ảnh map.

### ⌨️ PHÍM TẮT

- `Ctrl + Shift + O` — Mở/đóng Settings tổng. Trong bảng này có chỉnh `HUD SIZE`, `MAP CROP`, chọn map tròn/vuông, bật tắt riêng `MAP / ACTIVITY`, kéo `MOVE HUD` và tay nắm `SIZE ↘`.

Minimap dạng tròn tự xoay theo hướng nhìn của khủng long; phía trước luôn nằm ở đỉnh bản đồ.

> Tool chạy ngoài process game và không can thiệp vào game.

**Liên hệ:** [GitHub](https://github.com/wowiez)
