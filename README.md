...
- Bản cài đặt từ 1.7.6 tự kiểm tra GitHub Release một lần khi mở Home, tải nền và chỉ cập nhật khi người dùng xác nhận; portable không tự cập nhật.
...
```text
%LocalAppData%\Wowiez\IsleLiveMap\WebView2
```
...
Overlay Bearer token được lưu tại `%LocalAppData%\Wowiez\IsleLiveMap` sau khi mã hóa bằng DPAPI CurrentUser. Token:
...
```powershell
.\scripts\Package-Release.ps1 -Version 1.7.7
```
...
Thiết kế dự kiến cho auto-update:
...
4. App chỉ kiểm tra một lần sau khi Home đã mở, tải update ở nền, hỏi người dùng trước khi thoát và áp dụng bản mới.
