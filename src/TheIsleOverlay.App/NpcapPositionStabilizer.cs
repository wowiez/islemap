...
internal sealed class NpcapPositionStabilizer
{
    ...
    private const double SmoothingTimeSeconds = 0.18d;
    ...
    public bool TryStabilize(NpcapPositionSample sample, out NpcapPositionSample stabilized)
    {
        ...
        if (Math.Abs(sample.Location.X) < 1_000d && Math.Abs(sample.Location.Y) < 1_000d &&
            (Math.Abs(previous.Location.X) >= 1_000d || Math.Abs(previous.Location.Y) >= 1_000d))
        {
            return false;
        }
